...
export const generateForm = (dtoDir: string, formDir: string, model: any) => {
  ...
  Object.keys(model).forEach((key) => {
    ...
    fs.writeFileSync(path.join(formDir, `lib/${_name}.form.ts`), forms);
    fs.writeFileSync(path.join(dtoDir, `lib/${_name}.dto.ts`), dtos);

    formExports.push(`export * from './lib/${_name}.form';`);
    dtoExports.push(`export * from './lib/${_name}.dto';`);
  });
